...
if(request.getParameter("allow")!=null){
	squerry="UPDATE  "+TABLE_PREFIX+"comments SET  status= 0 WHERE id = '"+request.getParameter("allow")+"';";
	stmt.executeUpdate(squerry);
}
//Create the older/newer comments link
numPage=1;
...
squerry="SELECT COUNT(id) AS postcount FROM "+TABLE_PREFIX+"comments WHERE status = 1;";
...
squerry="SELECT * FROM "+TABLE_PREFIX+"comments WHERE status = 1 ORDER BY date_born DESC LIMIT "+(ManagePage-POSTS_TO_SHOW)+", "+POSTS_TO_SHOW+" ;";
rs = stmt.executeQuery(squerry);
c=0;
while (rs.next()) {
